...
def get_network():
    ...
    network = conv_2d(network, 256, 3, activation='elu', weights_init=truncated_normal(stddev=stddev3), bias=False) + biases[3]
    network = conv_2d(network, 256, 3, activation='elu', weights_init=truncated_normal(stddev=stddev3), bias=False) + biases[4]
    network = conv_2d(network, 256, 3, activation='elu', weights_init=truncated_normal(stddev=stddev3), bias=False) + biases[5]
